...
--[=[
	...
	(CopiedDictionary) -- s { ["SomethingInside"] = { ["A"] = 1, ["B"] = 1 } }
	```
...
--[=[
	...
	(TableKit.MergeDictionary(Dictionary, SecondDictionary)) -- s { ["A"] = 1, ["B"] = 2, ["C"] = 3, ["D"] = 4 }
	```
...
--[=[
	...
	(TableKit.Keys(Dictionary)) -- s {"A", "B", "C"}
	```
...
--[=[
	...
	(TableKit.Values(Dictionary)) -- s {1, 2, 3}
	```
...
--[=[
	...
	(TableKit.MergeArrays(FirstArray, SecondArray)) -- s {"A", "B", "C", D", "E", "F", "G", "H"}
	```
...
--[=[
	...
	(TableKit.Reconcile(toReconcile, template)) -- s { A = 9, B = 8, C = { D = "" }
	```
...
--[=[
	...
	(TableKit.IsArray(Array), TableKit.IsArray(Dictionary)) -- s true, false
	```
...
--[=[
	...
	(TableKit.IsDictionary(Array), TableKit.IsDictionary(Dictionary)) -- s false, true
	```
...
--[=[
	...
	(TableKit.ToString(DictionaryA)) -- s {
							--			[A]: Z
							--			[C]: Y
							--			[B]: X
							--		 }
...
--[=[
	...
	(TableKit.Filter(str, function(value)
		return value > "c"
	end))
	-- s {
 		[1] = "d",
...
--[=[
	...
	(TableKit.Some(array, even)) -- s true
	```
...
--[=[
	...
	(TableKit.IsFlat(Shallow)) -- s true
	(TableKit.IsFlat(Deep)) -- s false
	```
...
--[=[
	...
	(TableKit.Every(array, even)) -- s false
	(TableKit.Every(array, odd)) -- s false
	```
...
--[=[
	...
	(TableKit.HasKey(Dictionary, "Needle")) -- s true
	```
...
--[=[
	...
	(TableKit.HasValue(Array, "Has")) -- s true
	```
...
--[=[
	...
	(TableKit.IsEmpty(Empty), TableKit.IsEmpty(NotEmpty)) -- s true, false
	```
